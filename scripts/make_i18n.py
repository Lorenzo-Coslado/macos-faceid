...
TR = {
    ...
    "set.section.lock": {
        "en": "Lock Screen (experimental)", "fr": "Écran verrouillé (expérimental)",
        "es": "Pantalla de bloqueo (experimental)", "de": "Sperrbildschirm (experimentell)",
        "it": "Schermata di blocco (sperimentale)", "pt-BR": "Tela de bloqueio (experimental)",
        "nl": "Vergrendelscherm (experimenteel)", "ja": "ロック画面 (実験的)",
        "zh-Hans": "锁定屏幕 (实验性)", "ko": "잠금 화면 (실험적)",
        "ru": "Экран блокировки (эксперим.)",
    },
    "set.lock.toggle": {
        "en": "Face ID for the lock screen", "fr": "Face ID pour l'écran verrouillé",
        "es": "Face ID para la pantalla de bloqueo", "de": "Face ID für den Sperrbildschirm",
        "it": "Face ID per la schermata di blocco", "pt-BR": "Face ID para a tela de bloqueio",
        "nl": "Face ID voor vergrendelscherm", "ja": "ロック画面でFace ID",
        "zh-Hans": "锁屏使用 Face ID", "ko": "잠금 화면용 Face ID",
        "ru": "Face ID для экрана блокировки",
    },
    "set.lock.desc": {
        "en": "Reuses the PAM screensaver service. Your password always stays as a fallback.",
        "fr": "Réutilise le service PAM screensaver. Ton mot de passe reste toujours en repli.",
        "es": "Reutiliza el servicio PAM screensaver. Tu contraseña siempre queda como respaldo.",
        "de": "Nutzt den PAM-Dienst screensaver. Dein Passwort bleibt immer als Rückfall.",
        "it": "Riutilizza il servizio PAM screensaver. La password resta sempre come ripiego.",
        "pt-BR": "Reutiliza o serviço PAM screensaver. Sua senha permanece como alternativa.",
        "nl": "Hergebruikt de PAM-screensaverservice. Je wachtwoord blijft altijd als terugval.",
        "ja": "PAMのscreensaverサービスを再利用します。パスワードは常に予備として残ります。",
        "zh-Hans": "复用 PAM screensaver 服务。密码始终作为后备。",
        "ko": "PAM screensaver 서비스를 재사용합니다. 비밀번호는 항상 대체 수단으로 남습니다.",
        "ru": "Использует службу PAM screensaver. Пароль всегда остаётся запасным вариантом.",
    },
    "set.lock.warning": {
        "en": "Weaker than a password: a 2D webcam can be fooled by a photo.",
        "fr": "Moins sûr qu'un mot de passe : une webcam 2D peut être trompée par une photo.",
        "es": "Menos seguro que una contraseña: una webcam 2D puede engañarse con una foto.",
        "de": "Schwächer als ein Passwort: eine 2D-Webcam lässt sich mit einem Foto täuschen.",
        "it": "Meno sicuro di una password: una webcam 2D può essere ingannata da una foto.",
        "pt-BR": "Menos seguro que uma senha: uma webcam 2D pode ser enganada por uma foto.",
        "nl": "Zwakker dan een wachtwoord: een 2D-webcam is met een foto te misleiden.",
        "ja": "パスワードより安全性は低いです。2Dカメラは写真で騙される可能性があります。",
        "zh-Hans": "安全性低于密码：2D 摄像头可能被照片欺骗。",
        "ko": "비밀번호보다 안전하지 않습니다: 2D 웹캠은 사진에 속을 수 있습니다.",
        "ru": "Слабее пароля: 2D-камеру можно обмануть фотографией.",
    },
    "set.lock.trial": {
        "en": "Trial mode — removed on next restart until you confirm it works.",
        "fr": "Mode essai — retiré au prochain redémarrage tant que tu n'as pas confirmé.",
        "es": "Modo de prueba — se elimina al reiniciar hasta que confirmes que funciona.",
        "de": "Testmodus — beim nächsten Neustart entfernt, bis du es bestätigst.",
        "it": "Modalità di prova — rimossa al prossimo riavvio finché non confermi.",
        "pt-BR": "Modo de teste — removido ao reiniciar até você confirmar.",
        "nl": "Testmodus — bij herstart verwijderd totdat je bevestigt dat het werkt.",
        "ja": "お試しモード — 動作を確認するまで、次回の再起動で解除されます。",
        "zh-Hans": "试用模式 — 在你确认可用前，下次重启会移除。",
        "ko": "체험 모드 — 작동을 확인하기 전까지 다음 재시작 시 해제됩니다.",
        "ru": "Пробный режим — снимается при перезапуске, пока вы не подтвердите.",
    },
    "set.lock.confirm": {
        "en": "Confirm it works", "fr": "Confirmer que ça marche",
        "es": "Confirmar que funciona", "de": "Funktion bestätigen",
        "it": "Conferma che funziona", "pt-BR": "Confirmar que funciona",
        "nl": "Bevestigen dat het werkt", "ja": "動作を確認",
        "zh-Hans": "确认可用", "ko": "작동 확인", "ru": "Подтвердить работу",
    },
    "set.lock.confirmed": {
        "en": "Enabled permanently.", "fr": "Activé de façon permanente.",
        "es": "Activado permanentemente.", "de": "Dauerhaft aktiviert.",
        "it": "Attivato in modo permanente.", "pt-BR": "Ativado permanentemente.",
        "nl": "Permanent ingeschakeld.", "ja": "恒久的に有効化されました。",
        "zh-Hans": "已永久启用。", "ko": "영구적으로 활성화됨.",
        "ru": "Включено постоянно.",
    },
    "set.msg.lock.on": {
        "en": "Face ID enabled for the lock screen (trial).",
        "fr": "Face ID activé pour l'écran verrouillé (essai).",
        "es": "Face ID activado para la pantalla de bloqueo (prueba).",
        "de": "Face ID für den Sperrbildschirm aktiviert (Test).",
        "it": "Face ID attivato per la schermata di blocco (prova).",
        "pt-BR": "Face ID ativado para a tela de bloqueio (teste).",
        "nl": "Face ID ingeschakeld voor vergrendelscherm (proef).",
        "ja": "ロック画面でFace IDを有効にしました (お試し)。",
        "zh-Hans": "已为锁屏启用 Face ID（试用）。",
        "ko": "잠금 화면용 Face ID를 켰습니다 (체험).",
        "ru": "Face ID включён для экрана блокировки (проба).",
    },
    "set.msg.lock.off": {
        "en": "Face ID disabled for the lock screen.",
        "fr": "Face ID désactivé pour l'écran verrouillé.",
        "es": "Face ID desactivado para la pantalla de bloqueo.",
        "de": "Face ID für den Sperrbildschirm deaktiviert.",
        "it": "Face ID disattivato per la schermata di blocco.",
        "pt-BR": "Face ID desativado para a tela de bloqueio.",
        "nl": "Face ID uitgeschakeld voor vergrendelscherm.",
        "ja": "ロック画面のFace IDを無効にしました。",
        "zh-Hans": "已为锁屏停用 Face ID。",
        "ko": "잠금 화면용 Face ID를 껐습니다.",
        "ru": "Face ID отключён для экрана блокировки.",
    },
    "set.msg.lock.confirmed": {
        "en": "Lock screen Face ID confirmed.",
        "fr": "Face ID écran verrouillé confirmé.",
        "es": "Face ID de pantalla de bloqueo confirmado.",
        "de": "Face ID für Sperrbildschirm bestätigt.",
        "it": "Face ID schermata di blocco confermato.",
        "pt-BR": "Face ID da tela de bloqueio confirmado.",
        "nl": "Face ID voor vergrendelscherm bevestigd.",
        "ja": "ロック画面のFace IDを確認しました。",
        "zh-Hans": "已确认锁屏 Face ID。",
        "ko": "잠금 화면 Face ID가 확인되었습니다.",
        "ru": "Face ID для экрана блокировки подтверждён.",
    },
}
